...
public class PromoteCapture extends Move {
    ...
    public PromoteCapture(int startX, int startY, int endX, int endY, Piece promotePieceTo, Piece capturedPiece) {
    ...
    public Game revertMove(Game game) {
        ...
        if(game.getActiveColor() == 'w') {
            newBoard[startX][startY] = new WhitePawn(game.mg);
        } else {
            newBoard[startX][startY] = new BlackPawn(game.mg);
        }
    ...
    public String printMove(Game game) {
        ...
        return(pieceName + " " + startFile + tempStartY + " " + "captures at " + endFile + tempEndY + " and promotes to " + promotePieceTo.getName());
